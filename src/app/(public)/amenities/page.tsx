...
import { ShieldCheck, Utensils, Wifi, Lock, Zap, BookOpen, Shirt, Sparkles, HeartHandshake, CheckCircle2, Coffee, Sun, Heart, Award, Crown } from "lucide-react";
...
export default function AmenitiesPage() {
  ...
  return (
    <div className="min-h-screen bg-[#070709] text-slate-100">
      {/* Luxury Black & Gold Banner Header */}
      <section className="relative bg-gradient-to-r from-[#0d0d12] via-[#171410] to-[#0a0a0e] text-white py-20 px-4 sm:px-8 overflow-hidden border-b border-amber-500/20">
        <div className="absolute top-6 right-10 opacity-60 hidden md:block">
          <SquiggleWave color="#d4af37" width={140} height={28} />
        </div>
        <div className="absolute bottom-6 left-10 opacity-40 hidden md:block">
        ...
        <div className="max-w-4xl mx-auto text-center space-y-4 relative z-10">
          ...
          <h1 className="text-4xl sm:text-6xl font-black font-serif tracking-tight leading-tight text-white">
            Comprehensive <span className="text-gradient-gold-pure">Hostel Amenities</span>
          </h1>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-8 py-16 space-y-12">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {categories.map((cat) => (
            <Card
              key={cat.id}
              className="rounded-3xl border border-amber-500/25 shadow-xl hover:border-amber-400/60 transition-all duration-300 bg-[#0c0c10] overflow-hidden hover:-translate-y-1.5"
            >
              <CardHeader className="flex flex-row items-center gap-4 space-y-0 p-6 sm:p-8 pb-4 border-b border-amber-500/15">
                <div className={`w-14 h-14 rounded-2xl bg-gradient-to-tr ${cat.color} text-slate-950 flex items-center justify-center shadow-lg shadow-amber-500/20 font-black`}>
                  <cat.icon className="w-7 h-7 text-slate-950" />
                </div>
                <div>
